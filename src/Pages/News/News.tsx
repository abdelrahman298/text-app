...
export function Component() {
  ...
  const { data: SEO_DATA } = useFetchData(
    "SEO_DATA_News",
    `${import.meta.env.VITE_GET_SEO_DATA}/News`
  );
  ...
  const { isPending, isError, isPaused, data } = useFetchPaginatedData(
    "News",
    page,
    `${import.meta.env.VITE_NEWS}?limit=${
  ...
  return (
    <section className=" site_container  pt-20  pb-32">
      <HelmetTags
        ...
        canonical={`${i18n.language}/news`}
      />
      <Heading color={defaultData.color} className={"text-center"}>
        {t("heading")}{" "}
      </Heading>

      <div className="all__news--wrapper w-full grid-auto-fit my-10 ">
        {isError || isPaused ? (
