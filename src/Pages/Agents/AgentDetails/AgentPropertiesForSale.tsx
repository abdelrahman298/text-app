...
const AgentPropertiesForSale = memo(function AgentPropertiesForSale({
  t,
  type = "broker",
  userProps,
}: {
  ...
  const finalData = type === "agent" ? userProps : data?.for_sale.content;
  // console.log(
    ...
    <>
      <div className="Agent__properties--wrapper w-full grid grid-cols-1 gap-[35px]  ">
        {(isError || isPaused) && type === "agent" ? (
          <ErrorMessage message={t("AgentPropertiesForSale.ErrorMessage")} />
        ) : isPending && type === "agent" ? (
          <Loader className="h-40" />
        ) : finalData?.length === 0 ? (
          <NoItemsMessage
            message={t("AgentPropertiesForSale.NoItemsMessage")}
          />
        ) : (
...
export default AgentPropertiesForSale;
